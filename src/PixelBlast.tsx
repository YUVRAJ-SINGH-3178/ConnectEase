...
const createTouchTexture = (): TouchTexture => {
  ...
  let last: NormalizedTouch | null = null;
  const maxAge = 64;
  let radius = 0.1 * size;
  ...
  const addTouch = (norm: NormalizedTouch) => {
    let force = 0;
    let vx = 0;
    let vy = 0;
    if (last) {
...
type NormalizedTouch = Pick<TouchPoint, "x" | "y">;
...
type TouchTexture = {
  ...
  addTouch: (norm: NormalizedTouch) => void;
  update: () => void;
  radiusScale: number;
  size: number;
};
...
const isEffectPass = (pass: unknown): pass is EffectPassLike =>
  typeof pass === "object" && pass !== null && "effects" in pass;

const updatePassUniformTime = (pass: unknown, time: number) => {
  if (!isEffectPass(pass) || !pass.effects) return;
...
const disposeRuntime = (runtime: ThreeRuntime, container: HTMLElement) => {
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    const needsReinitKeys: Array<keyof PixelBlastConfig> = [
      "antialias",
      "liquid",
      "noiseAmount",
    ];
    ...
    if (mustReinit) {
      if (threeRef.current) {
        disposeRuntime(threeRef.current, container);
      ...
      // In dev, add a faint tint to help confirm the container is visible
      try {
        const isDev = Boolean(
        ...
        if (isDev) {
          container.style.background =
            "linear-gradient(90deg, rgba(124,93,255,0.02), rgba(0,0,0,0))";
        }
      } catch {
        // ignore env detection issues
      }
      ...
      try {
        ...
        console.warn("PixelBlast setSize failed", error);
      }

      // Extra debug: confirm DOM presence after a short delay
      setTimeout(() => {
        console.debug("PixelBlast DOM check", {
          containerClient: {
      ...
      const mapToPixels = (e: PointerEvent): PointerPixels => {
      ...
      const animate = () => {
        ...
        if (composer) {
          ...
          composer.passes.forEach((pass) =>
            updatePassUniformTime(pass, uniforms.uTime.value)
          );
          composer.render();
      ...
      raf = requestAnimationFrame(animate);
      console.debug("PixelBlast renderer started", {
